...
knitr::opts_chunk$set(include = F, error = T)
library(ggmcmc)
library(ggpubr)
library(tidyverse)
library(ggthemes)
...
if(rerun_models == F){
  ...
  source('RScripts/OptimisingModels.R')
}
```
...
if(rerun_models == T){
  source('RScripts/Model1.R', echo = F)
}
...
```{r Model1pMCMC, include = T, echo = F}
source('RScripts/pMCMCfunctions.R')
model<- Model1[[1]]
fixed_names_list<- colnames(model$Sol)[grep('species.', colnames(model$Sol), invert = T)]
...
FixedEffectsTables<-GenerateFixedEffectsTable(model=model,link="poisson",
                         fixed_names=fixed_names_list,
                         fixed_diffinc = c("all"),
                         pvalues = "include")
knitr::kable(FixedEffectsTables[1], caption = "Table S1.1: Estimates of Fixed Effects")
knitr::kable(FixedEffectsTables[2], caption = "Table S1.2: Comparison Between Fixed Effects")
```
...
if(rerun_models == T){
  source('RScripts/Model2.R', echo = F)
}
...
```{r Model2pMCMC, include = T, echo = F}
model<- Model2[[1]]
fixed_names_list<- colnames(model$Sol)[grep('species.', colnames(model$Sol), invert = T)]
...
if(rerun_models == T){
  source('RScripts/Model3.R', echo = F)
}
...
Model3MeanPlot
```
```{r Model3pMCMC, include = T, echo = F}
model<- Model3[[1]]
fixed_names_list<- colnames(model$Sol)[grep('species.', colnames(model$Sol), invert = T)]
...
if(rerun_models == T){
  source('RScripts/Model4.R', echo = F)
}
...
Model4MeanPlot
```
```{r Model4pMCMC, include = T, echo = F}
model<- Model4[[1]]
fixed_names_list<- colnames(model$Sol)[grep('species.', colnames(model$Sol), invert = T)]
...
if(rerun_models == T){
  source('RScripts/Model5.R', echo = F)
}
...
Again, just about overlaps with 0, so not clear. 
```{r Model5pMCMC, include = T, echo = F}
model<- Model5[[1]]
fixed_names_list<- colnames(model$Sol)[grep('species.', colnames(model$Sol), invert = T)]
...
if(rerun_models == T){
  source('RScripts/Model6.R', echo = F)
}
...
```{r Model6pMCMC, include = T, echo = F}
model<- Model6[[1]]
fixed_names_list<- colnames(model$Sol)[grep('species.', colnames(model$Sol), invert = T)]
...
if(rerun_models == T){
  source('RScripts/Model7.R', echo = F)
}
...
```{r Model7pMCMC, include = T, echo = F}
model<- Model7[[1]]
fixed_names_list<- colnames(model$Sol)[grep('species.', colnames(model$Sol), invert = T)]
...
if(rerun_models == T){
  source('RScripts/Model8.R', echo = F)
}
...
```{r Model8pMCMC, include = T, echo = F}
model<- Model8[[1]]
fixed_names_list<- colnames(model$Sol)[grep('species.', colnames(model$Sol), invert = T)]
