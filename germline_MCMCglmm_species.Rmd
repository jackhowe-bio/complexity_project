...
Tested using a model accounting for phylogeny (P1), from 1M to 10M iterations, testing with thinning of 100 and 1000, and burnin from 500000 to 1000000. 

```{r}
testing_args <- expand.grid(n_itts = seq(1000000, 10000000, length = 5), n_thin = seq(100,1000, length = 2), n_burn = seq(500000, 1000000, length = 4))
...
P1_parallel_test <- mclapply(1:nrow(testing_args), function(i, n_itts,n_burn, n_thin){
 MCMCglmm(cell_number ~FissionOrBuddingObserved_Species-1, #-1 here removes the intercept equivalent to 0 in brms
                ...
                family ="poisson",data = df,prior=p2, nitt=n_itts[i], burnin=n_burn[i], thin=n_thin[i], verbose = T,pr=T)
}, mc.set.seed=T, mc.cores = 6, n_itts = testing_args$n_itts, n_burn = testing_args$n_burn, n_thin = testing_args$n_thin)

options(scipen = 8) 
names(P1_parallel_test)<- paste('_it:',testing_args$n_itts,'_thin:',testing_args$n_thin,'_burnin:',testing_args$n_burn, sep = '')
summary(P1_parallel_test)


autocorrelation_tests<- as.data.frame(lapply(P1_parallel_test, function(m) autocorr(m$VCV, lags = min(testing_args$n_itts)/2000))) #conduct autocorrelation tests

#autocorrelation_tests$lag<- row.names(autocorrelation_tests)

autocorrelation_tests_long<- 
  
  autocorrelation_tests %>%
  pivot_longer(cols = everything(),  
               names_to = c("iterations", "thinning", "burnin","a","b"),
               names_pattern = "X_it\\.(.*)_thin\\.(.*)_burnin\\.(.*)\\.(.*)\\.(.*)", 
               values_to = 'autocor') %>%
  filter(a == 'species', b == 'species')

autocorrelation_tests_long$iterations<- as.numeric(autocorrelation_tests_long$iterations)

ggplot(autocorrelation_tests_long, aes(x = iterations, y = abs(autocor), colour = burnin, shape = thinning)) + geom_point() + geom_line() + theme_minimal()
```
