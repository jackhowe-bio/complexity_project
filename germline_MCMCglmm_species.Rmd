...
```{r}
testing_args <- expand.grid(n_itts = seq(1000000, 10000000, length = 5), n_thin = seq(100,1000, length = 3), n_burn = seq(500000, 10000000, length = 4))
...
glimpse(autocorrelation_tests)

ggplot(autocorrelation_tests_long, aes(x = iterations, y = autocorrelation, colour = burnin)) + geom_point(aes(shape = thinning), size = 2, alpha = 0.9, position = position_dodge(width = 100)) + geom_line(aes(group = interaction(burnin, thinning))) + theme_minimal()
```
